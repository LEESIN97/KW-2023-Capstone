    ...
    tf_broadcaster(this)
{
    ...
    tag_edge_size = declare_parameter("size", 0.173, descr("default tag size", true));

    // get tag names, IDs and sizes
    const auto ids = declare_parameter("tag.ids", std::vector<int64_t>{9, 14}, descr("tag ids", true));
    const auto frames = declare_parameter("tag.frames", std::vector<std::string>{"base", "object"}, descr("tag frame names per id", true));
    const auto sizes = declare_parameter("tag.sizes", std::vector<double>{0.162, 0.162}, descr("tag sizes per id", true));
